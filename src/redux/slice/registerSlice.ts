export type State = {
  isRegisterStatus: boolean;
  isRegisterDone: boolean;
  phone: string;
  password: string;
  firstName: string;
  lastName: string;
};

const initState: State = {
  isRegisterStatus: false,
  isRegisterDone: true,
  phone: "",
  password: "",
  firstName: "",
  lastName: "",
};

export enum registerEnum {
  START_REGISTER = "START_REGISTER",
  END_REGISTER_SUCCESS = "END_REGISTER_SUCCESS",
  END_REGISTER_FAILED = "END_REGISTER_FAILED",
}

export type registerAction = {
  type: registerEnum;
  payload?: any;
};

export const registerSlice = (state = initState, action: registerAction) => {
  const newState = { ...state };
  switch (action.type) {
    case registerEnum.START_REGISTER:
      newState.isRegisterDone = false;
      newState.phone = action.payload.phone;
      newState.password = action.payload.password;
      newState.firstName = action.payload.firstName;
      newState.lastName = action.payload.lastName;
      break;
    case registerEnum.END_REGISTER_SUCCESS:
      newState.isRegisterDone = true;
      newState.isRegisterStatus = true;
      break;
    case registerEnum.END_REGISTER_FAILED:
      newState.isRegisterDone = true;
      newState.isRegisterStatus = false;
      break;
  }
  return newState;
};

export const handleStartRegisterRedux = (payload: {
  phone: string;
  password: string;
}) => ({
  type: registerEnum.START_REGISTER,
  payload,
});
export const handleEndRegisterSuccessRedux = () => ({
  type: registerEnum.END_REGISTER_SUCCESS,
});
export const handleEndRegisterFailedRedux = () => ({
  type: registerEnum.END_REGISTER_FAILED,
});
